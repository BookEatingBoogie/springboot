...
public class StoryGenerationController {
    ...
    @PostMapping("/story")
    public ResponseEntity<?> requestStory(@RequestBody String choice) {
        if (page == 5 || choice == null) { return ResponseEntity.badRequest().build();}

        //page가 0~5사이일 경우 중간부 생성 함수 호출.
        if (page < 5 && page > 0) {
            System.out.println("page = " + page);
            try {
                StoryReturnDTO response = storyGenerationService.generateContent(choice, page, charId,"user", creationId);
                page--;
        ...
        } else if (page == 0) { //page=0이면 엔딩 생성 및 정제 함수 호출.
            System.out.println("page = " + page);
            //전체 스토리 생성.
            try {
                String contentUrl = storyGenerationService.generateSaveStory(choice, charId, "user", creationId);
